...
- **AI**: OpenRouter with `deepseek/deepseek-v4-flash` for all AI features (Matt's decision 2026-06-12, supersedes the earlier AI Gateway plan). Model and provider pinned in `convex/lib/ai.ts`, key in `OPENROUTER_API_KEY` (Convex env + `.env.local`). Agents are Convex actions writing proposals, never direct mutations. `agentRuns` stores prompt, sources, proposal diff, decision, actor.
