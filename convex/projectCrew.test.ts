...
test("talent can be booked for one production without going into People, and added later", async () => {
  const { t, ids, asA } = await setup();
  const bookingId = await asA.mutation(api.projectCrew.add, {
    projectId: ids.project,
    kind: "talent",
  ...
  let list = await asA.query(api.projectCrew.listForProject, { projectId: ids.project });
  ...
  expect(await asA.query(api.people.list, { kind: "talent" })).toHaveLength(0);
  expect(await asA.query(api.people.list, { kind: "crew" })).toHaveLength(1);

  // Their details are edited on the booking while they are not in People.
  await asA.mutation(api.projectCrew.update, { id: bookingId, phone: "07700 900222" });

  const personId = await asA.mutation(api.projectCrew.saveToPeople, { id: bookingId });
  const talent = await asA.query(api.people.list, { kind: "talent" });
  expect(talent).toHaveLength(1);
  expect(talent[0]).toMatchObject({ _id: personId, name: "Jo Patel", phone: "07700 900222" });
  list = await asA.query(api.projectCrew.listForProject, { projectId: ids.project });
  expect(list[0]).toMatchObject({ personId, inPeople: true, booked: true });
  const booking = await t.run(async (ctx) => ctx.db.get(bookingId));
  expect(booking?.name).toBeUndefined();

  // Now in People, the booking no longer holds their details.
  await expect(
    asA.mutation(api.projectCrew.update, { id: bookingId, phone: "07700 900333" })
  ).rejects.toThrow("People record");
});

test("somebody on one production only goes on the call sheet and is not a role to book", async () => {
  const { t, ids, asA } = await setup();
  await asA.mutation(api.projectCrew.add, {
    projectId: ids.project,
    name: "Alex Grip",
  ...
  const day = await t.run(async (ctx) =>
    ctx.db.insert("shootDays", { orgId: ids.org, projectId: ids.project, date: "2026-10-01", locationIds: [] })
  );
  await asA.mutation(api.callSheets.generateFromProject, { shootDayId: day });
  const sheet = await asA.query(api.callSheets.getCurrent, { shootDayId: day });
  expect(sheet?.data.crew[0]).toMatchObject({ name: "Alex Grip", role: "Grip", phone: "07700 900444" });

  const list = await asA.query(api.projectCrew.listForProject, { projectId: ids.project });
  expect(list[0].booked).toBe(true);
});
